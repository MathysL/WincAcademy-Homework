// dirty JavaScript source code but filed with imb moviesbase meta
//
...
// template to clone en display in row of td index.html
var template = document.querySelector('#row');
for (var i = 0; i < movies.length; i += 1) {
    var cat = movies[i];
    var clone = template.content.cloneNode(true);
    var cells = clone.querySelectorAll('td');
    cells[0].textContent = cat.title;
    cells[1].textContent = cat.Year;
    cells[2].textContent = cat.imdbID;
    cells[3].textContent = cat.Type;
    var filmPoster = cells[4].textContent = cat.Poster;
